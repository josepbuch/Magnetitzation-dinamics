...
def neural_conv_prove_optima_xarxa_5():
    Net = Sequential()
    Net.add(Input((64, 64, 1)))
    
    Net.add(Conv2D(filters=8, kernel_size=[5, 5], padding='same'))
    Net.add(PReLU())
    Net.add(BatchNormalization())
    Net.add(MaxPooling2D(pool_size=2))
    
    Net.add(Conv2D(filters=16, kernel_size=[5, 5], padding='same'))
    Net.add(PReLU())
    Net.add(BatchNormalization())
    Net.add(MaxPooling2D(pool_size=2))

    Net.add(Conv2D(filters=32, kernel_size=[5, 5], padding='same'))
    Net.add(PReLU())
    Net.add(BatchNormalization())
    Net.add(MaxPooling2D(pool_size=2))
...
Net = neural_conv()
epoch = 75
alpha_param = np.transpose(np.array([param_train[:,0],param_train[:,3]]))
